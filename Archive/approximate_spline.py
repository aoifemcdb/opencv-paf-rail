...
from colour_shape_sensing.curvature_methods import *
from colour_shape_sensing.calibration import Calibration


### THIS SCRIPT IS OLD!! 19.06.23 ###
...
def process_image(image, lower_color, upper_color, kernel_size=5):
    ...
    #sample curve to remove wrong edge of contour - need to find a way to do this not manually
    x_new = x_new[200:-500]
    y_new_smooth = y_new_smooth[200:-500]
...
def test_curve():
    #this does the calibration every time but really it only needs to be done for one batch of images so change the code here at some point
    ...
    file_path = '../colour_shape_sensing/experiment_images_010623/70mm_test_iter1.jpg'

    color_name = 'green'
    real_width = 30  # mm
    real_length = 30  # mm
    # calibration_filepath = './colour_shape_sensing/experiment_images_010623/110mm_test_iter1.jpg'
    calibration_image = cv2.imread(calibration_filepath)
    # plt.figure()
    # plt.imshow(calibration_image)
    # plt.show()
    colour_boundaries = ColorBoundaries()
    lower_color, upper_color = colour_boundaries.get_color_boundaries(color_name)
    ...
    img, mask, x_new, y_new_smooth = process_image(undistorted_image, lower_color, upper_color)

    spline_curve_downsampled = downsample_spline_curve(x_new,y_new_smooth,100)
    plot_image_and_spline(img, mask, x_new, y_new_smooth)
...
def main():
    curve = test_curve()
    curvature, _ = circle_fitting_method(curve)
