...
#     transformed_points = [(point[0][0] - x_offset, point[0][1] - y_offset) for point iqn points]
#
...
def pre_calibration(calibration_filepath):
    calibration = Calibration()
    real_width = 10  # mm
    real_length = 20  # mm
    color = 'blue'
    checkerboard_size = (5, 7)
    camera_matrix, dist_coeffs = calibration.calibrate_camera(calibration_filepath, checkerboard_size)
    return calibration, camera_matrix, dist_coeffs
...
def main():
    ...
    calibration_filepath = './experiment_images_260623/checkerboard_calibration/*.jpg'

    calibration, camera_matrix, dist_coeffs = pre_calibration(calibration_filepath)
    ...
    while True:
        ...
        image, mask, x_new, y_new_smooth = shape_sensing.process_video(undistorted_image, 'green')

        if x_new is not None and y_new_smooth is not None:
            spline_curve_downsampled = shape_sensing.downsample_spline_curve(x_new, y_new_smooth, 100)
            print(spline_curve_downsampled)
